      ...
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          <button
            ...
            className="p-2 rounded-lg bg-white/5 border border-white/8 text-slate-300 hover:text-white transition-colors"
          >
            <FontAwesomeIcon icon={faArrowLeft} className="text-xs" />
          </button>
          <div>
            <h1 className="text-xl font-bold text-white">Nouveau ticket</h1>
            <p className="text-slate-300 text-sm">Décrivez votre problème, notre équipe vous répond sous 24h</p>
          </div>
        </div>

        {sent ? (
          <div className="bg-[#1C2D4F] rounded-xl border border-green-500/20 p-10 text-center">
            ...
            <p className="text-slate-300 text-sm mb-6">Notre équipe vous répondra dans les meilleurs délais.</p>
            <button
          ...
          <div className="bg-[#1C2D4F] rounded-xl border border-white/8 p-6 max-w-2xl">
            <div className="space-y-4">
              <div>
                <label className="block text-slate-300 text-xs font-medium uppercase tracking-wider mb-2">Sujet *</label>
                <select
                  value={newForm.sujet}
                ...
                >
                  {subjectOptions.map((o) => (
                    <option key={o.value} value={o.value} disabled={!o.value} className="bg-[#1C2D4F]">{o.label}</option>
                  ))}
                </select>
              </div>
              <div>
                <label className="block text-slate-300 text-xs font-medium uppercase tracking-wider mb-2">Description *</label>
                <textarea
                  value={newForm.message}
                  onChange={(e) => setNewForm({ ...newForm, message: e.target.value })}
                  placeholder="Décrivez votre problème en détail..."
                  rows={6}
                  className="w-full bg-white/5 border border-white/10 rounded-lg px-4 py-3 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-blue-500/50 resize-none"
                />
              </div>
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-slate-400 text-xs font-mono">{selected.id.slice(0, 8).toUpperCase()}</span>
            ...
            <p className="text-slate-400 text-xs">Ouvert {timeAgo(selected.createdAt)}</p>
          </div>
        </div>
            ...
            {selected.messages.map((m, i) => (
              <div key={i} className={`flex gap-3 ${!m.isAdmin ? "flex-row-reverse" : ""}`}>
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-[11px] mt-1">{m.user.prenom} {m.user.nom} · {timeAgo(m.createdAt)}</p>
                </div>
              </div>
            ))}
          </div>
          {selected.status !== "RESOLU" && (
            <div className="p-5 border-t border-white/8">
              <textarea
                ...
                rows={3}
                className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-sm text-white placeholder-slate-400 focus:outline-none focus:border-blue-500/50 resize-none mb-3"
              />
              <div className="flex justify-end">
                <button
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-300 text-sm mt-0.5">Vos demandes auprès de l&apos;équipe BYS Permis</p>
        </div>
        <button
              ...
              <div className="flex items-start justify-between gap-4">
                <div>
                  <div className="flex items-center gap-2 mb-1.5">
                    <span className="text-slate-400 text-xs font-mono">{t.id.slice(0, 8).toUpperCase()}</span>
                  ...
                  <p className="text-slate-400 text-xs mt-0.5">{t.messages.length} message(s) · Ouvert {timeAgo(t.createdAt)}</p>
                </div>
